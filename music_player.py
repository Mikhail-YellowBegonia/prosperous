    ...
    def draw(self, engine):
        ...
        for i in range(self.width):
            ...
            for row in range(self.height):
                ...
                char = " "
                if row < h: char = "█"
                elif row == h: char = chars[int((val * self.height - h) * 8)]
...
def main():
    ...
    with Live(fps=30, logic_fps=60) as live:
        ...
        lyric_stack = VStack(pos=(15, 5), gap=0, children=[
        ...
        while live.running:
            for key in live.poll():
                ...
                if key == "UP": target_index = max(0.0, target_index - 1.0)
                if key == "DOWN": target_index = min(len(songs_data) - 1.0, target_index + 1.0)
