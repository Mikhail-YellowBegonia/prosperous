...
[x] **基础容器**：`Panel`（边框容器，padding，get_child_origin）、`Box`（纯边框，无标题）。
[x] **布局容器**：`VStack` / `HStack`，子组件自动排列，支持 gap、align、reverse。
[x] **UI 原子**：`Text`（静态/lambda，支持 width 和 align）、`ProgressBar`、`LogView`。
...
[x] **入口抽象**：`Live` 上下文管理器，封装三线程、限速（fps/logic_fps）、cleanup、live.stop()。
[x] **帧管理**：`live.frame()` 自动 clear/flush，`live.add()` / `live.remove()` 管理场景。
[x] **焦点系统**：`FocusManager` 焦点栈，`push_group()` / `pop_group()` 支持 Modal 隔离；`live.add()` 自动注册 focusable 组件。
[x] **Theme 系统**：`DEFAULT_THEME` + `set_theme()`，组件按类型查表获取默认参数（padding、style、focus_style 等）。
[ ] **滚动支持**：LogView 等组件的滚动浏览逻辑（已知技术债，单独立项）。
...
[ ] **单元测试**：布局数学、样式系统、输入解析、组件状态（pytest，已由子 Agent 建立框架）。
[ ] **缓冲区测试**：draw() 后 screen_prepare 内容断言，覆盖渲染逻辑。
[ ] **快照测试**：完整场景字符数组对比，防止布局回归。
